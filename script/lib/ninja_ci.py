...
from common import get_default_args_array,build_target_path, application_name, get_vcvars_path
...
class BuildHelper():
    ...
    @property
    def out_path(self):
        return build_target_path(self._src_root, self._build_target)
    ...
    def run_ninja(self, target=['chrome']):
        try:
    ...
    def get_signed_files(self):
        need_signed_files = []
        for root_, _, files_ in os.walk(self.out_path):
            for file_ in files_:
                if self.filename_is_match(root_, file_) :
                    need_signed_files.append(os.path.join(root_, file_))
        print('need signed file num = %s' % len(need_signed_files))
        return need_signed_files

    def sign_browser_files(self, need_signed_files):
        ...
        set_vcvars_cmd = get_vcvars_path()
        ...
        for signed_file in need_signed_files:
            ...
            print('End sign browser file %s ' %signed_file)
    ...
    def run_build_target(self):
        ...
        self.run_ninja()
        expect_object = os.path.join(self.out_path, application_name())
        assert os.path.exists(expect_object), '%s is not exists ' % expect_object
        if IS_WIN:
            self.run_ninja(['mini_installer'])
            expect_object = os.path.join(self.out_path, 'mini_installer.exe')
            assert os.path.exists(expect_object), '%s is not exists ' % expect_object
            self.sign_browser_files([expect_object])
        else:
            print('Other platform no need signed for app')
