    ...
    R: Read + Unpin + Send + Sync + 'static,
{
    ...
    for header in httparse_res.headers.iter() {
        let str = std::str::from_utf8(header.value)?;
        if str.to_header_values().is_ok() {
            res.append_header(header.name, str);
        } else {
            warn!("found non ascii header: {} -- {}", header.name, str);
            let value = utf8_percent_encode(str, NON_ALPHANUMERIC).to_string();
