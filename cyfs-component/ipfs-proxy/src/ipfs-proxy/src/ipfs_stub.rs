...
impl IPFSStub {
    ...
    pub async fn set_port(&self, ipfs_gateway_port: u16, ipfs_api_port: u16, swarm_port: u16) -> BuckyResult<()> {
    ...
    pub async fn start(&self) -> bool {
        ...
        if let Ok(_) = self.run_ipfs(&vec!["daemon".as_ref()]) {
            let mut wait_secs = 2;
            loop {
                info!("waiting ipfs daemon run after {} secs...", wait_secs);
                async_std::task::sleep(Duration::from_secs(wait_secs)).await;
                if self.ipfs_is_running().await {
                    info!("ipfs daemon running");
                    return true;
                }
